...
if (isset($_POST['submit_manufacturer'])) {
  ...
  if ($conn->query($sql) === TRUE) {
    ...
    <script>Swal.fire({icon:'success',title:'Success!',text:'Manufacturer added!'}).then(() => {window.location.href='?page=view_manufacturer';});</script>";
...
if (isset($_POST['submit_product'])) {
  ...
  if ($conn->query($sql) === TRUE) {
    echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script>
    <script>Swal.fire({icon:'success',title:'Success!',text:'Product added!'}).then(() => {window.location.href='?page=view';});</script>";
  } else { echo "Error: " . $conn->error; }
}
...
if ($page == 'insert') {
  ...
  <div class="card p-4 shadow">
    ...
    <form method="POST">
      ...
      <div class="mb-3"><label class="form-label">Manufacturer ID</label><input type="number" name="manufacturer_id" class="form-control" required></div>
      <button type="submit" name="submit_product" class="btn btn-primary"><i class="fas fa-plus"></i> Add</button>
    </form>
  </div>';
} elseif ($page == 'view') {
    ...
    echo "<div class='table-responsive'>
      <table class='table table-striped'>
        <thead class='table-dark'>
          <tr>
            <th>ID</th><th>Name</th><th>Price</th><th>Manufacturer ID</th><th>Manufacturer Name</th><th>Action</th>
          </tr>
        </thead>
        <tbody>";
...
function deleteRecord(id) {
  Swal.fire({title:'Are you sure?', text:'This will be deleted!', icon:'warning', showCancelButton:true, confirmButtonText:'Yes, delete it!'})
